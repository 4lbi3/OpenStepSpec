...
3. Draft proposal materials **one file at a time** in this order—\`proposal.md\`, \`design.md\` (when needed), \`tasks.md\`, then spec deltas—and stop after each file so the user can review and validate it before continuing; only run \`openspec validate <id> --strict\` if the user requests it or after they approve the file.
